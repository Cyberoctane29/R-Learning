---
title: An R Markdown document converted from "C:\Users\saswa\Downloads\module-4-hoa-aesthetics-and-visualizations.ipynb"
...
By now, you are pretty familiar with this data set. But you can refresh your memory with the `head()` and `colnames()` functions. Run two code chunks below to get a sample of the data and also preview all the column names:
...
Previously, you used `geom_point` to make a scatter plot comparing lead time and the number of children. Now, you will use `geom_bar` to make a bar chart in this code chunk:
...
This code chunk also creates a bar chart with 'distribution_channel' on the x-axis and 'count' on the y-axis. But it also includes data from the 'deposit_type' column as color-coded sections of each bar. There is a legend explaining what each color represents on the right side of the visualization.
